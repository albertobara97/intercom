...
class Intercom_DWT(Intercom_empty):
    ...
    def send(self, indata):
        ...
        self.coeffs_c0,self.coeffs_c01 = pywt.dwt(self.flat, "db5", mode='per')
        
        self.flat = indata[:,1].flatten()
        self.coeffs_c1,self.coeffs_c11 = pywt.dwt(self.flat, "db5", mode='per')
        ...
        self.coeffs_c01 = np.array(self.coeffs_c0,dtype=np.uint32)
        self.coeffs_c1 = np.array(self.coeffs_c1,dtype=np.uint32)
        self.coeffs_c11 = np.array(self.coeffs_c1,dtype=np.uint32)
        ...
        self.send_bitplane(self.coeffs_c0,self.coeffs_c01,self.max_NOBPTS-1, 0)
        self.send_bitplane(self.coeffs_c0,self.coeffs_c01, self.max_NOBPTS-2, 0)

        self.send_bitplane(self.coeffs_c1,self.coeffs_c11, self.max_NOBPTS-1, 1)
        self.send_bitplane(self.coeffs_c1,self.coeffs_c11, self.max_NOBPTS-2, 1)

        for bitplane_number in range(self.max_NOBPTS-3, last_BPTS, -1):
            self.send_bitplane(self.coeffs_c0,self.coeffs_c01, bitplane_number, 0)
            self.send_bitplane(self.coeffs_c1,self.coeffs_c11, bitplane_number, 1)
    ...
    def send_bitplane(self, indata,indata2, bitplane_number, channel):
        ...
        envio=np.append(indata,indata2)
        bitplane = (envio[:,] >> bitplane_number//self.number_of_channels) & 1
        
        if np.any(bitplane): 
            bitplane= bitplane.astype(np.uint8)
            bitplane= np.packbits(bitplane)
            message = struct.pack(self.packet_format, self.recorded_chunk_number, bitplane_number, *bitplane, channel)
    ...
    def receive_and_buffer(self):
        ...
        bitplane=np.asarray(bitplane,dtype=np.uint16)
         
        bitplane= bitplane.flatten()
        indice=len(bitplane)/2
        indice=int(indice)
        array1=bitplane[:indice]
        array2=bitplane[:indice]
        idwt= pywt.idwt(array1,array2, 'db5', mode='per')
        idwt = idwt.astype(np.uint32)
        self._buffer[received_chunk_number % self.cells_in_buffer][:, channel] |= (idwt << received_bitplane_number//self.number_of_channels)
